...
fn build_kjv(out_dir: &str, data_root: &Path) {
    let kjv_dir = data_root.join("sources/kjv");
    println!("cargo:rerun-if-changed={}", kjv_dir.display());
...
fn build_web(out_dir: &str, data_root: &Path) {
    let web_dir = data_root.join("sources/web");
    println!("cargo:rerun-if-changed={}", web_dir.display());
    let path = data_root.join("sources/web/eng-web.usfx.xml");
...
fn build_sv(out_dir: &str, data_root: &Path) {
    let sv_dir = data_root.join("sources/sv");
    println!("cargo:rerun-if-changed={}", sv_dir.display());
    let path = data_root.join("sources/sv/STV.xml");
...
fn main() {
    ...
    // data/ lives at the workspace root, one level above bible-core/
    let manifest_dir = std::env::var("CARGO_MANIFEST_DIR").unwrap();
    let data_root = std::path::Path::new(&manifest_dir).join("../data");

    build_kjv(&out_dir, &data_root);
    build_web(&out_dir, &data_root);
    build_sv(&out_dir, &data_root);
}
